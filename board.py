...
class GoBoard(object):
    ...
    def _detect_capture(self, move: GO_POINT, color: GO_COLOR) -> List[GO_POINT]:
        """
    Check whether the move can result in capturing stones.
    If yes, return the list of captured stones.
    """
        captured = []
        
        opp_color = opponent(color)

    # Get the neighbors of the move point
        neighbors = self.neighbors_of_color(move, opp_color)

    # Check each neighbor to determine captured stones
        for neighbor in neighbors:
            opp_block = self._block_of(neighbor)
            if not self._has_liberty(opp_block):
            # The opponent's block has no liberty, so it's captured
                captures = list(where1d(opp_block))
    ...
    #     if not GoBoardUtil.generate_legal_moves(self.board, self.current_player):
    #         self.current_player = opponent(self.current_player)
    def checkwinner(self) -> GO_COLOR:
    ...
    def check_winner(self) -> GO_COLOR:
        result1 = self.detect_five_in_a_row()
        result2 = EMPTY
    ...
    # def has_four_in_list(self, lst) -> GO_COLOR:
    #     """
    #     Returns BLACK or WHITE if any five in a rows exist in the list.
    #     EMPTY otherwise.
    #     """
    ...
    # def find_moves_to_block_four(self, color: GO_COLOR) -> List[GO_POINT]:
    #     blocking_moves = []

    #     for r in self.rows:
    #         blocking_moves.extend(self.find_blocking_moves(r, color))
    #     for c in self.cols:
    #         blocking_moves.extend(self.find_blocking_moves(c, color))
    #     for d in self.diags:
    #         blocking_moves.extend(self.find_blocking_moves(d, color))

    #     return blocking_moves

    # def find_blocking_moves(self, lst, color: GO_COLOR) -> List[GO_POINT]:
    #     """
    #     Returns a list of moves that block a four in a row in the list.
    #     """
    #     blocking_moves = []
    ...
    #             board_copy.play_move(stone, color)
    #             if board_copy.detect_four_in_a_row() == EMPTY:
    #                 blocking_moves.append(stone)

    #     return blocking_moves
    
    def _get_captured_stones(self, move: GO_POINT, color: GO_COLOR) -> list:
        captured_stones = []
        ...
        # Check each neighbor to determine captured stones
        for neighbor in neighbors:
            if self.get_color(neighbor) == opponent(color):
                block = self._block_of(neighbor)
                if not self._has_liberty(block):
                    # The opponent's block has no liberty, so it's captured
                    captured_stones.extend(list(where1d(block)))
